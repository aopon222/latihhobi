...
@section('content')
<section class="ecourse-header" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; padding: 40px 20px; text-align: center; margin-bottom: 40px;">
    <div class="ecourse-container">
        <h1 style="font-size: 2.5rem; margin: 0 0 10px 0;">E-Course Film & Konten Kreator</h1>
        <p style="font-size: 1.1rem; margin: 0;">Kembangkan kreativitas dalam pembuatan film dan konten digital</p>
    </div>
</section>

<section class="products-section" style="padding: 40px 20px;">
    <div class="products-container" style="max-width: 1200px; margin: 0 auto;">
        <h2>Kursus Film & Konten Kreator Tersedia</h2>
                    ...
                    <div class="product-card" style="background: white; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
                        ...
                        <div class="product-info" style="padding: 15px;">
                            ...
                            <div style="margin: 10px 0;">
                                <span style="font-size: 1.3rem; font-weight: bold; color: #00f2fe;">Rp {{ number_format($course->price, 0, ',', '.') }}</span>
                            </div>
                            <div style="display: flex; gap: 10px; margin-top: 15px;">
                                ...
                                <form action="{{ route('ecourse.addToCart', $course->id_course) }}" method="POST" style="flex: 1;">
                                    @csrf
                                    <button type="submit" style="width: 100%; padding: 8px 12px; border: none; border-radius: 4px; background: #00f2fe; color: #333; cursor: pointer;">Tambah ke Keranjang</button>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        @else
            <div style="text-align: center; padding: 40px 20px; color: #999;">
                <p>Belum ada kursus film & konten kreator tersedia saat ini.</p>
            </div>
        @endif
    </div>
</section>
