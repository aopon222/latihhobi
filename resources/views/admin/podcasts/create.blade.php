...
@section('title', 'Tambah Podcast - Admin LatihHobi')

@section('admin-content')
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;">
    <h1 style="font-size:2rem;font-weight:700;color:#2563eb;">Tambah Podcast</h1>
    ...
    <form method="POST" action="{{ route('admin.podcasts.store') }}">
        ...
        <div style="margin-bottom:24px;">
            ...
            <textarea name="description" rows="5" required
                      ...
                      style="width:100%;padding:12px;border:1px solid {{ $errors->has('description') ? '#ef4444' : '#d1d5db' }};border-radius:8px;font-size:14px;resize:vertical;">{{ old('description') }}</textarea>
            @error('description')
                <p style="color:#ef4444;font-size:12px;margin-top:4px;">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div style="margin-bottom:24px;">
            ...
            <input type="text" name="topics" value="{{ old('topics') }}"
